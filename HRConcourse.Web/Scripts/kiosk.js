...
$(document).ready(function () {
    ...
    $.tools.validator.addEffect("customErrors", function (errors, event) {
        ...
        // Notify of page errors
        var message = { event: 'page-error' };
        window.top.postMessage(message, '*'); //send the message and target URI
...
function GetDocument(revisionId) {
    ...
    $.ajax({
        type: "POST",
        url: getHierFullyQualifiedURL("GetKioskDocument") + '?revisionId=' + revisionId,
        success: function (data) {
            data = data.result;
            if (data.success === true) {
                $mainPanel.html(data.content);
                ...
                markSectionAsActive($('#' + revisionId));
                // hide loading dialog
                hideLoadingDialog();
            } else {
                // hide loading dialog
...
function GetDocumentInstance(revisionId) {
    // scroll to top of the page
    $('html, body').animate({ scrollTop: 0 }, 'slow');
    showLoadingDialog();
    ...
    $.ajax({
        type: "POST",
        url: getFullyQualifiedURL("GetDocumentInstance") + '?fillRequestId=' + revisionId,
        success: function (data) {
            data = data.result;
            if (data.success === true) {
                $mainPanel.html(data.content);
...
function showDocumentCompleteSection(revisionId) {
    ...
    $.ajax({
        type: "POST",
        url: getFullyQualifiedURL("GetDocumentCompleted") + "?fillRequestId=" + revisionId,
        success: function (data) {
            data = data.result;
            if (data.success === true) {
...
function showFaxDocumentLaterSection(revisionId) {
    ...
    $.ajax({
        type: "POST",
        url: getFullyQualifiedURL("GetFaxDocumentLater") + "?fillRequestId=" + revisionId,
        success: function (data) {
            data = data.result;
            // insert the page inside the main section
            $mainPanel.html(data);
...
function selectDocFormGroup(revisionId) {
    ...
    /// <param name="revisionId" type="string">The ID of the document to select.</param>
    /// <returns type="void"></returns>

    var $documentGroupSelection = $('#' + DOCUMENT_GROUP_SELECTION);
    var $docSelection = $documentGroupSelection.find('#' + GROUP_ITEM_PREFIX + revisionId);
...
function GetDocumentInGroup(revisionId, groupTypeId) {
    ...
    if ($docSelection.length > 0 && $groupLink.length > 0) {
        ...
        var actionOnClick = "GetDocumentInstance('" + revisionId + "')";

        // update id, onclick action, title and displayed name on the documents list
        $groupLink.attr('id', revisionId)
                  .attr('onclick', actionOnClick);

        $groupLink.find('div')
                .attr('id', DOC_TITLE_PREFIX + revisionId)
                .attr('title', newDocTitle)
                .html(newDocTitle);

        // load the selected document
        GetDocumentInstance(revisionId);
    } else {
        hideLoadingDialog();
    }
}

function changeDocGroupingSelection(revisionId) {
    ...
    /// <param name="revisionId" type="string">The ID of the actual selected document in the gruop.</param>
    /// <returns type="void"></returns>

    showLoadingDialog();
    // find the current document section
    $documentLink = $documentListContainer.find('#' + revisionId);
...
function skipPage() {
    ...
    if ($documentSection !== undefined) {
        var revisionId = $documentSection.attr('id').slice(DOC_DIV_PREFIX.length);
...
function checkAndLoad(pageId, revisionId, href) {
    ...
    /// <param name="revisionId" type="string">The ID of the corresponding document.</param>
    ...
    $documentSection = $('#' + DOC_DIV_PREFIX + revisionId);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success === true) {
                    ...
                    preloadDocumentPages(revisionId, pageId, MAX_PRELOADED_PAGES);
                } else {
                    // hide the loading dialog
                    hideLoadingDialog();
                    // show the NEXT button
...
function setPageSettings(pageSection, docSection) {
    ...
    // setup any given values to override
    if (documentValues) {
      setupDocumentValues(pageSection);
    }
...
function preloadDocumentPages(revisionId, pageId, count) {
    ...
    if (count > 0) {
        ...
        if (nextLinkIndex < numberOfPages) {
            ...
            if (nextLinkIndex + 1 < numberOfPages) {
                ...
                preloadDocumentPages(revisionId, nextPageId, count - 1);
            }
        }
    }
}
...
function pageSubmitCustomAction(form) {
    ...
    var revisionId = $documentSection.attr('id').slice(DOC_DIV_PREFIX.length); // id="DOC_DIV_PREFIX + revisionId"

    // Process page field values
    pageFieldArrayValues = pageFieldArrayValues.concat(processPageFieldValues(form, form.serializeArray()));
    $.extend(documentValues, getDocumentValues());

    submitSuccess(revisionId);
}

function processPageFieldValues(pageSection, values) {
  $.each(values, function (index, field) {
    // Input fields
    pageSection.find(FIELD_CONTAINER).find('input, select').each(function () {
      if ($(this).attr('name') == field.name) {
        field.id = $(this).parents(FIELD_CONTAINER).attr('id');
      }
    });
  });
...
function setupDocumentValues(pageSection) {
  ...
  fieldContainers.find(TEXT_PASS_EMAIL_NUMBER_SELECT_INPUTS).each(function () {
    ...
    if (documentValues[fieldName]) {
      $(this).val(documentValues[fieldName]);
    }
  });
  fieldContainers.find(DATEPICKER_INPUT).each(function () {
    var fieldName = $(this).attr('name');
    if (documentValues[fieldName]) {
      var strDate = documentValues[fieldName].replace(/\-/ig, '/').split('T')[0].split('.')[0];
  ...
  fieldContainers.find(RADIO_INPUT).each(function () {
    var fieldName = $(this).attr('name');
    if (documentValues[fieldName] && documentValues[fieldName] === $(this).val()) {
  ...
  fieldContainers.find(CHECKBOX_INPUT).each(function () {
    var fieldName = $(this).attr('name');
    if (documentValues[fieldName]) {
      var initialValues = documentValues[fieldName].split(',');
...
function submitSuccess(revisionId) {
...
function submitPageComments(pageId) {
    var pageComments = $('#' + pageId + '-comments').find('#UserComments').val().replace(/\n/g, '__XR2_New_Line__');
    if (pageComments !== undefined) {
        $.ajax({
            type: "POST",
            url: getFullyQualifiedURL("SubmitPageComments") + '?pageId=' + pageId + '&userComments=' + pageComments,
            success: function (data) {
                data = data.result;
                if (data.success === true) {
    ...
    if ($documentSection.attr(DOCUMENT_STATUS_ATTR) === READONLY_STATUS) {
        var serializedComments = $('#' + pageId + '-comments').find('#UserComments').serialize();
        $.ajax({
            type: "POST",
            url: getFullyQualifiedURL("SubmitForm") + '?pageId=' + pageId,
...
function onSuccess(e) {
    ...
    var revisionId = e.response.fillRequestId;
    // load the document complete page
    showDocumentCompleteSection(revisionId);
}
...
function showOriginalDocument(revisionId, docTitle) {

    showLoadingDialog();
    $.ajax({
        type: "POST",
        url: getFullyQualifiedURL("GetFullDocument") + "?revisionId=" + revisionId,
        success: function (data) {
            data = data.result;
            if (data.success) {
...
function getPendingDocumentIndex(revisionId) {
    /// <summary>Return the index of the provided document on the Pending document list.</summary>
    /// <param name="revisionId" type="string">The ID of the document to get the index.</param>
    /// <returns type="Number"></returns>

    var pendingDocumentLinks = $pendingDocumentList.find(DOC_MENU_LINKS).children();
    return pendingDocumentLinks.index($('#' + revisionId));
}

function isDocumentPending(revisionId) {
    /// <summary>Return true if the document is listed on the Pending section.</summary>
    /// <param name="revisionId" type="string">The ID of the document to check.</param>
    /// <returns type="bool"></returns>

    return getPendingDocumentIndex(revisionId) !== -1;
}

function getSubmittedDocumentIndex(revisionId) {
    ...
    return submittedDocumentLinks.index($('#' + revisionId));
}

function isDocumentSubmitted(revisionId) {
    /// <summary>Return true if the document is listed on the Submitted section.</summary>
    /// <param name="revisionId" type="string">The ID of the document to check.</param>
    /// <returns type="bool"></returns>

    return getSubmittedDocumentIndex(revisionId) !== -1;
}

function getApprovedDocumentIndex(revisionId) {
    ...
    return approvedDocumentLinks.index($('#' + revisionId));
}

function isDocumentApproved(revisionId) {
    /// <summary>Return true if the document is listed on the Approved section.</summary>
    /// <param name="revisionId" type="string">The ID of the document to check.</param>
    /// <returns type="bool"></returns>

    return getApprovedDocumentIndex(revisionId) !== -1;
}
...
function moveToPendingSection(revisionId) {
    /// <summary>Moves a document from the submitted or approved sections to the Pending section.</summary>
    /// <param name="revisionId" type="string">The ID of the document to move.</param>
    /// <returns type="bool"></returns>

    var documentLink = $documentListContainer.find('#' + revisionId);
    var isSubmitted = isDocumentSubmitted(revisionId);
    if (isSubmitted || isDocumentApproved(revisionId)) {
...
function loadFinishPage() {
    ...
    var data = "";
    $.each(pageFieldArrayValues, function (index, field) { data += field.id + '=' + field.value + '&'; });

    $.ajax({
        type: "POST",
        url: getFullyQualifiedURL("SubmitForm") + "?revisionId=" + revisionId,
        data: data,
        success: function (data) {
            // hide loading dialog
            hideLoadingDialog();
            ...
            var message = { event: 'finished', entryId: data.entryId, documentValues: getDocumentValues() };
            window.top.postMessage(message, '*'); //send the message and target URI
        },
        error: function () {
            // hide loading dialog
...
function setDocumentValues(values) {
  if ($.isPlainObject(values) && !$.isEmptyObject(values)) {
    documentValues = values;
    setupDocumentValues($pageForm);
  }
}

function getDocumentValues() {
  var res = {};
  $(pageFieldArrayValues).each(function (index, field) { res[field.name] = field.value; });
  return res;
}

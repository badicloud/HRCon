...
/*****************************************************************************************/
/********************    MODIFIED TO WORK WITH IE - MEDIA STYLE   ************************/
/****************    MODIFIED TO USE CUSTOM EVENTS - $printDocument  *********************/
/*****************************************************************************************/
...
(function ($) {
    $.fn.printPage = function (options) {
        ...
        */
        function loadPrintDocument(el, pluginOptions) {
            ...
            $("#printMessageBox").animate({ opacity: 1 }, 300, function () {
                if (CheckIsOpera()) {
                    printOnNewPage(el, pluginOptions);
                } else {
        ...
        function printOnNewPage(el, pluginOptions) {
            var url = (pluginOptions.url) ? pluginOptions.url : $(el).attr(pluginOptions.attr);

            var win = window.open('about:blank');
            win.document.write('<html><head></head><body>');
            win.document.write('<iframe id="printPage" frameBorder="0" align="center" src="' + url + '" onload="printFrame()" style="width: 970px; height: 1300px;"></iframe>');
            win.document.write('<scr' + 'ipt>function bindPageComplete() {$(\'#printPage\')[0].contentWindow.$printDocument.bind(\'printPageComplete\', function () { printFrame(); });} function printFrame(){window.focus();window.print();}</sc' + 'ript></body></html>');
            win.document.close();
            window.focus();
            unloadMessage();

            //var owin = window.open(url, '', 'width=1,height=1');
            //owin.opener.focus();    //lose its focus(): it matters
            //owin.print();
            //owin.close();
            //owin = null;

            //var win2 = window.open(url, '', 'width=1,height=1');
            //win2.blur();
            //win2.print();
            //win2.bind("load", function () { printit(); });
            //window.focus();
            //unloadMessage();
        }
        ...
        */
        function addIframeToPage(el, pluginOptions) {
            ...
            if (!$('#printPage')[0]) {
                ...
                //$('#printPage').bind("load", function () { bindPageComplete(); });
                $('#printPage').bind("load", function () { setPrintImagesReady(); });
            } else {
        ...
        function bindPageComplete() {
            ...
            $('#printPage')[0].contentWindow.$printDocument.bind('printPageComplete', function () { updateMessage('Document processed'); printit(); });
        }
        ...
        */
        ...
        /*
        * Update the message box with a progress indication
        */
